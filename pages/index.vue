<template>
  <div>
    <!--Nav-->
    <nav
      class="bg-gray-200 fixed py-4 px-2 mt-0 w-full z-10 top-0 shadow-md"
      style="opacity: 0.9;"
    >
      <div class="mx-auto flex flex-wrap items-center">
        ...
        >
          <nuxt-link
            class="no-underline text-blue hover:text-blue hover:no-underline text-2xl pl-2"
            to="/"
          >
            Big Data & Health
          </nuxt-link>
        ...
        <!--          >-->
        <!--            <li class="mr-3" v-for="article in articles" :key="article.slug">-->
        <!--              <nuxt-link-->
        <!--                :to="article.slug"-->
    ...
    <footer>
      <div
        class="flex justify-between items-center border-t-2 fixed bottom-0 w-full px-4 bg-gray-100"
      >
        <div class="flex items-center">
          ...
          >
            <img class="mr-3" src="logo_nwo.png" width="40" />
          </a>
          <a
            target="_blank"
...
export default {
  ...
  async asyncData({ $content, slug }) {
    const articles = await $content('topics').fetch()
    return { articles }
  },
}
</script>
